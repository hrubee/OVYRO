/**
 * Route-handler plumbing for the seller lead inbox API (task OVYRO-9e62).
 *
 ...
 *   - `NotFoundError`        → 404 NOT_FOUND          (ownership, never disclosed)
 *   - `LeadTransitionError`  → 409 INVALID_TRANSITION (from `@/lib/leads`)
 *   - anything with numeric `status` + string `code`   → itself
 *   - anything else          → 500 INTERNAL_ERROR (details never leaked)
 *
 * This mirrors the seller listings envelope (`../../listings/_lib/http.ts`)
 * deliberately: each dashboard API feature owns its own copy so feature scopes
 * stay independent, and the structural {@link isAppError} check keeps the mapper
 * open to new typed errors (auth, leads-core) without a growing switch.
 */
...
/**
 * Lead missing, or on a listing the caller does not own. Ownership failures
 * surface as 404 (not 403) so a seller cannot probe for the existence of leads
 * that are not theirs (task guard: "ownership 404 otherwise").
 */
...
export function mapErrorToResponse(err: unknown): NextResponse {
  ...
  console.error("Unhandled dashboard leads route error", err);
